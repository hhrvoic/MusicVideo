...
class JsonToVideoParser {
    
    static func extractVideoDataFromJson(videoDataObject: AnyObject) -> [MusicVideo] {
        
        if let vidDataJSON = videoDataObject as? JSONDictionary
        {
            var videos = [MusicVideo]()
            
            if let feeds = vidDataJSON["feed"] as? JSONDictionary, entries = feeds["entry"] as? JSONArray {
                ...
                for (index, data) in entries.enumerate() {
                    ...
                    let currentVideo = MusicVideo(vRank: index + 1, vName: vName, vRights: vRights, vPrice: vPrice, vImageUrl: vImageUrl, vArtist: vArtist, vVideoUrl: vVideoUrl, vImid: vImid, vGenre: vGenre, vLinkToiTunes: vLinkToiTunes, vReleaseDte: vReleaseDte)
